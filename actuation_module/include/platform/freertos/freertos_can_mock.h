#ifndef PLATFORM__FREERTOS__CAN_MOCK_H_
#define PLATFORM__FREERTOS__CAN_MOCK_H_
...
inline std::size_t mock_send_fail_from{std::numeric_limits<std::size_t>::max()};
...
inline void reset_recorded_can_frames()
{
  ...
  mock_send_fail_from = std::numeric_limits<std::size_t>::max();
}
...
{
  RecordedCanFramesLock lock;
  if (recorded_can_frames_size >= mock_send_fail_from) {
    common::logger::log_error("FreeRTOS CAN mock send forced to fail");
    return false;
  }
...
{
  common::logger::log_error("CAN-FD is not supported by the FreeRTOS CAN mock");
  return false;
}

}  // namespace common::can::platform

#endif  // PLATFORM__FREERTOS__CAN_MOCK_H_
