...
# reused CMakeLists.txt has no CACHE var for it -- see common/dds/
...
# Task 21 once set CONFIG_DDS_MAX_MSG_SIZE/CONFIG_DDS_MAX_REXMIT_MSG_SIZE/
# CONFIG_DDS_FRAGMENT_SIZE=434/434/348 here, matching freertos_x5h/
# CMakeLists.txt on the CR52 firmware side, for the 462-byte RPMsg-netif MTU
# that link used at the time. That MTU is now 1500B, so those three flags
# are gone from both -DCMAKE_C_FLAGS/-DCMAKE_CXX_FLAGS strings below; only
# CONFIG_DDS_DISABLE_MULTICAST=1 remains, since this reused CMakeLists.txt
# has no CACHE var for it.
